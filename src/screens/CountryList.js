...
const onShare = async () => {
  ...
  try {
    ...
    analytics
      .hit(new PageHit("Country_Cases_Share"))
    ...
    const result = await Share.share({
      message: `Corona Virus Updates (Top 3 Countries) - ${"\n"}${"\n"}${
        ...
        response.data.countries_stat[0].cases
      }${"\n"}Recovered - ${
        response.data.countries_stat[0].total_recovered
      }${"\n"}Active Cases - ${
        response.data.countries_stat[0].active_cases
      }${"\n"}Deaths - ${response.data.countries_stat[0].deaths}${"\n"}${"\n"}${
        response.data.countries_stat[1].country_name
      }${"\n"}${"\n"}Confirmed Cases -${
        response.data.countries_stat[1].cases
      }${"\n"}Recovered - ${
        response.data.countries_stat[1].total_recovered
      }${"\n"}Active Cases - ${
        response.data.countries_stat[1].active_cases
      }${"\n"}Deaths - ${response.data.countries_stat[1].deaths}${"\n"}${"\n"}${
        response.data.countries_stat[2].country_name
      }${"\n"}${"\n"}Confirmed Cases -${
        response.data.countries_stat[2].cases
      }${"\n"}Recovered - ${
        response.data.countries_stat[2].total_recovered
      }${"\n"}Active Cases - ${
        response.data.countries_stat[2].active_cases
      }${"\n"}Deaths - ${
        response.data.countries_stat[2].deaths
          ...
          renderItem={({ item }) => {
            return (
              <DetailCountries
                ...
                deltaConfirmed={item.new_cases}
                deltaDeaths={item.new_deaths}
              />
            );
          }}
          keyExtractor={(data) => {
