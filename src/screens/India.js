...
const onShare = async () => {
  ...
  try {
    ...
    analytics
      .hit(new PageHit("Share_India_Cases"))
      .then(() => console.log("Success"))
      .catch((e) => console.log(e.message));
    const result = await Share.share({
      message: `Corona Virus Updates For India - ${"\n"}${"\n"}Confirmed Cases -${
        response.data.total_values.confirmed
      }${"\n"}Recovered - ${
        response.data.total_values.recovered
      }${"\n"}Active Cases - ${
        response.data.total_values.active
      }${"\n"}Deaths - ${
        response.data.total_values.deaths
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={{ paddingBottom: normalize(280) }}>
          ...
          >
            <Image
              style={styles.image}
              source={require("../../assets/img/flag-round-250.png")}
            ></Image>
          ...
          >
            Updated- {results.lastupdatedtime}
          </Text>
          <View style={{ alignItems: "center" }}>
            {results.length === 0 ? (
              ...
              <View style={styles.ButtonData}>
                ...
                >
                  Confirmed - {ConvertToIndianSystem(results.confirmed)} ( +
                  {ConvertToIndianSystem(results.deltaconfirmed)} )
                </Text>
              </View>
            )}
            ...
            {results.length === 0 ? null : (
              <View style={styles.ButtonData}>
                ...
                >
                  Active - {ConvertToIndianSystem(results.active)}
                </Text>
              </View>
            )}
            ...
            {results.length === 0 ? null : (
              <View style={styles.ButtonData}>
                ...
                >
                  Deaths - {ConvertToIndianSystem(results.deaths)} ( +
                  {ConvertToIndianSystem(results.deltadeaths)} )
                </Text>
              </View>
            )}

            {results.length === 0 ? null : (
              <View style={styles.ButtonData1}>
                <Text
                  style={{
                    fontFamily: "Bebas Neue",
                    fontSize: normalize(23),
                ...
                >
                  Recovered - {ConvertToIndianSystem(results.recovered)} ( +
                  {ConvertToIndianSystem(results.deltarecovered)} )
                </Text>
              </View>
            )}
          </View>
            ...
            >
              <View style={styles.button}>
                <Text style={styles.buttonText}>
                  Check State/District-wise Data
                </Text>
              </View>
            </TouchableOpacity>
          </View>
